...
def chat_interface(db, user_id, user_gender):
    ...
    # Initialize chatbot and recommender
    if "chatbot" not in st.session_state:
        st.session_state.chatbot = AshaBot()
    
    if "recommender" not in st.session_state and db is not None:
        st.session_state.recommender = SessionRecommender(db)
        ...
        if "recommender" in st.session_state:
            ...
            if recommendations:
                ...
                for rec in recommendations:
                    ...
                    with st.sidebar.expander(f"{session['session_title']} ({relevance:.2%} match)"):
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if db is not None:
            try:
                user = db.users.find_one({"_id": ObjectId(user_id)})
                profile_complete = user is not None and "profile" in user and user["profile"]
            except Exception as e:
            ...
            if not profile_complete:
                ...
                if db is not None:
                    try:
                        ...
                        st.warning(f"Could not display profile: {e}")
                ...
                try:
                    ...
                    if recommendations:
                        for rec in recommendations:
                            ...
                            if session:
                                with st.expander(f"{session['session_title']} ({rec['relevance_score']:.2%} match)"):
                                    st.write(f"**Description**: {session.get('description', 'No description available')}")
